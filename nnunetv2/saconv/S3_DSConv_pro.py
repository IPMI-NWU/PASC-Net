...
class DSConv_pro(nn.Module):
    def __init__(
        ...
        self.gn_offset = nn.GroupNorm(kernel_size, kernel_size)
    ...
    def forward(self, input: torch.Tensor):
        ...
        elif self.morph == 1 or self.morph == 3:
    ...
    if morph == 0:
        ...
        for index in range(1, center + 1):
            y_offset_new_[center + index] = (
                y_offset_new_[center + index - 1] + offset[center + index]
            )
            y_offset_new_[center - index] = (
                y_offset_new_[center - index + 1] + offset[center - index]
            )
        ...
        offset = einops.rearrange(offset, "b k w h -> k b w h")
        xy_offset_new_ = offset.detach().clone()

        # The center position remains unchanged and the rest of the positions begin to swing
        # This part is quite simple. The main idea is that "offset is an iterative process"

        xy_offset_new_[head] = 0
        xy_offset_new_[tail] = 0

        for index in range(1, center):
            xy_offset_new_[head + index] = (
                    xy_offset_new_[head + index - 1] + offset[head + index]
            )
            xy_offset_new_[tail - index] = (
                    xy_offset_new_[tail - index + 1] + offset[tail - index]
            )

        xy_offset_new_ = einops.rearrange(xy_offset_new_, "k b w h -> b k w h")

        y_new_ = y_new_.add(xy_offset_new_.mul(extend_scope))
        x_new_ = x_new_.sub(xy_offset_new_.mul(extend_scope))
        ...
        for index in range(1, center + 1):
            x_offset_new_[center + index] = (
                x_offset_new_[center + index - 1] + offset[center + index]
            )
            x_offset_new_[center - index] = (
                x_offset_new_[center - index + 1] + offset[center - index]
            )
        ...
        y_new_ = y_new_.add(xy_offset_new_.mul(extend_scope))
        x_new_ = x_new_.add(xy_offset_new_.mul(extend_scope))
