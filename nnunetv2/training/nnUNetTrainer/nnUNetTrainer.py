...
from nnunetv2.utilities.center_line_loss import cldice
...
class nnUNetTrainer(object):
    ...
    def get_con5(self,logit,d):
        ...
        dir_array=dir_array.permute(0, 3, 1, 2)
        return dir_array
    ...
    def get_con(self,logit,d):
        ...
        # 利用张量操作替代循环
        i_indices, j_indices = torch.nonzero(logit_binary, as_tuple=True)[1:]
        dir_array = torch.zeros([batch_size, shp[0], shp[1], 9], device=device)
        dir_array[:, i_indices, j_indices, 0] = img_pad[:, i_indices, j_indices]
        dir_array[:, i_indices, j_indices, 1] = img_pad[:, i_indices, j_indices + board]
        dir_array[:, i_indices, j_indices, 2] = img_pad[:, i_indices, j_indices + 2*board]
        dir_array[:, i_indices, j_indices, 3] = img_pad[:, i_indices + board, j_indices]
        dir_array[:, i_indices, j_indices, 4] = img_pad[:, i_indices + board, j_indices + board]
        dir_array[:, i_indices, j_indices, 5] = img_pad[:, i_indices + board, j_indices + 2*board]
        dir_array[:, i_indices, j_indices, 6] = img_pad[:, i_indices + 2*board, j_indices]
        dir_array[:, i_indices, j_indices, 7] = img_pad[:, i_indices + 2*board, j_indices + board]
        dir_array[:, i_indices, j_indices, 8] = img_pad[:, i_indices + 2*board, j_indices + 2*board]
        dir_array=dir_array.permute(0, 3, 1, 2)
        return dir_array
    ...
    def conloss(self, logit, target, d):
        ...
        loss = nn.BCEWithLogitsLoss()(logit, tar)
        return loss
    ...
    def train_step(self, batch: dict) -> dict:
        ...
        with autocast(self.device.type, enabled=True) if self.device.type == 'cuda' else dummy_context():
            output,con1,con3 = self.network(data)
            del data
            l = 0.7*self.loss(output, target)+0.1*(self.conloss(con1,target,1)+0.1*self.conloss(con3,target,3))+0.1*self.clloss(output,target)
    ...
    def validation_step(self, batch: dict) -> dict:
        ...
        # So autocast will only be active if we have a cuda device.
        with autocast(self.device.type, enabled=True) if self.device.type == 'cuda' else dummy_context():
#            output,connection = self.network(data)
#            del data
#            l = 0.8*self.loss(output, target)+0.2*self.conloss(connection,target,1)
    ...
    def on_epoch_end(self):
        ...
        if (current_epoch + 1) % self.save_every == 0 and current_epoch != (self.num_epochs - 1):
            self.save_checkpoint(join(self.output_folder, str(current_epoch)+'checkpoint_latest.pth'))
